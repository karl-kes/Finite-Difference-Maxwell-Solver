...
public:
    ...
    double static constexpr cfl_factor{ 0.75 };
    std::size_t static constexpr total_steps{ 1000 };
    ...
    std::size_t const pml_thickness{ static_cast<std::size_t>(std::cbrt(size) / 8.0) };
    ...
    double static constexpr pml_alpha_max{ 0.3 };

    std::size_t output_interval() const { return total_steps / 100; }
};
