...
, thickness_padded_{ (thickness_ + doubles_per_alignment_ - 1) & ~(doubles_per_alignment_ - 1) }
, Nx_{ config.Nx + 1 }
, Ny_{ config.Ny + 1 }
, Nz_{ config.Nz + 1 }
...
, alpha_max_{ config.pml_alpha_max } {
    ...
    // 1. Allocate the 64-byte aligned block
    std::size_t const coeff_block_size{ num_coeff_arrays_ * thickness_padded_ };
    double* ptr{ static_cast<double*>( alignedAlloc( alignment_bytes_, coeff_block_size * sizeof(double) ) ) };
    if ( !ptr ) { throw std::bad_alloc(); }
    
    // Safely zero everything out (including padding)
    std::fill_n( ptr, coeff_block_size, 0.0 );
    coeff_memory_block_.reset (ptr );
...
void PML::update_B_psi(
    ...
    double* RESTRICT psi_Byx{ psi_Byx_.get() };
    double* RESTRICT psi_Bzx{ psi_Bzx_.get() };
    double* RESTRICT psi_Bxy{ psi_Bxy_.get() };
    double* RESTRICT psi_Bzy{ psi_Bzy_.get() };
    double* RESTRICT psi_Bxz{ psi_Bxz_.get() };
    double* RESTRICT psi_Byz{ psi_Byz_.get() };
    ...
    for ( std::size_t z = 0; z < Nz_ - 1; ++z ) {
        for ( std::size_t y = 0; y < Ny_ - 1; ++y ) {
            for ( std::size_t d = 0; d < t; ++d ) {
                ...
                psi_Byx_[pi_lo] = b_Bx[d] * psi_Byx_[pi_lo] + c_Bx[d] * dEy_dx_lo;
                psi_Bzx_[pi_lo] = b_Bx[d] * psi_Bzx_[pi_lo] + c_Bx[d] * dEz_dx_lo;
                ...
                psi_Byx_[pi_hi] = b_Bx[t - 1 - d] * psi_Byx_[pi_hi] + c_Bx[t - 1 - d] * dEy_dx_hi;
                psi_Bzx_[pi_hi] = b_Bx[t - 1 - d] * psi_Bzx_[pi_hi] + c_Bx[t - 1 - d] * dEz_dx_hi;
    ...
    for ( std::size_t z = 0; z < Nz_ - 1; ++z ) {
        for ( std::size_t x = 0; x < Nx_ - 1; ++x ) {
            for ( std::size_t d = 0; d < t; ++d ) {
                ...
                psi_Bxy_[pi_lo] = b_By[d] * psi_Bxy_[pi_lo] + c_By[d] * dEx_dy_lo;
                psi_Bzy_[pi_lo] = b_By[d] * psi_Bzy_[pi_lo] + c_By[d] * dEz_dy_lo;
                ...
                psi_Bxy_[pi_hi] = b_By[t - 1 - d] * psi_Bxy_[pi_hi] + c_By[t - 1 - d] * dEx_dy_hi;
                psi_Bzy_[pi_hi] = b_By[t - 1 - d] * psi_Bzy_[pi_hi] + c_By[t - 1 - d] * dEz_dy_hi;
    ...
    for ( std::size_t y = 0; y < Ny_ - 1; ++y ) {
        for ( std::size_t x = 0; x < Nx_ - 1; ++x ) {
            for ( std::size_t d = 0; d < t; ++d ) {
                ...
                psi_Bxz_[pi_lo] = b_Bz[d] * psi_Bxz_[pi_lo] + c_Bz[d] * dEx_dz_lo;
                psi_Byz_[pi_lo] = b_Bz[d] * psi_Byz_[pi_lo] + c_Bz[d] * dEy_dz_lo;
                ...
                psi_Bxz_[pi_hi] = b_Bz[t - 1 - d] * psi_Bxz_[pi_hi] + c_Bz[t - 1 - d] * dEx_dz_hi;
                psi_Byz_[pi_hi] = b_Bz[t - 1 - d] * psi_Byz_[pi_hi] + c_Bz[t - 1 - d] * dEy_dz_hi;
...
void PML::update_E_psi(
    ...
    double* RESTRICT psi_Eyx{ psi_Eyx_.get() };
    double* RESTRICT psi_Ezx{ psi_Ezx_.get() };
    double* RESTRICT psi_Exy{ psi_Exy_.get() };
    double* RESTRICT psi_Ezy{ psi_Ezy_.get() };
    double* RESTRICT psi_Exz{ psi_Exz_.get() };
    double* RESTRICT psi_Eyz{ psi_Eyz_.get() };
    ...
    for ( std::size_t z = 1; z < Nz_; ++z ) {
        for ( std::size_t y = 1; y < Ny_; ++y ) {
            for ( std::size_t d = 0; d < t; ++d ) {
                ...
                psi_Eyx_[pi_lo] = b_Ex[d] * psi_Eyx_[pi_lo] + c_Ex[d] * dBy_dx_lo;
                psi_Ezx_[pi_lo] = b_Ex[d] * psi_Ezx_[pi_lo] + c_Ex[d] * dBz_dx_lo;
                ...
                psi_Eyx_[pi_hi] = b_Ex[t - 1 - d] * psi_Eyx_[pi_hi] + c_Ex[t - 1 - d] * dBy_dx_hi;
                psi_Ezx_[pi_hi] = b_Ex[t - 1 - d] * psi_Ezx_[pi_hi] + c_Ex[t - 1 - d] * dBz_dx_hi;
    ...
    for ( std::size_t z = 1; z < Nz_; ++z ) {
        for ( std::size_t x = 1; x < Nx_; ++x ) {
            for ( std::size_t d = 0; d < t; ++d ) {
                ...
                psi_Exy_[pi_lo] = b_Ey[d] * psi_Exy_[pi_lo] + c_Ey[d] * dBx_dy_lo;
                psi_Ezy_[pi_lo] = b_Ey[d] * psi_Ezy_[pi_lo] + c_Ey[d] * dBz_dy_lo;
                ...
                psi_Exy_[pi_hi] = b_Ey[t - 1 - d] * psi_Exy_[pi_hi] + c_Ey[t - 1 - d] * dBx_dy_hi;
                psi_Ezy_[pi_hi] = b_Ey[t - 1 - d] * psi_Ezy_[pi_hi] + c_Ey[t - 1 - d] * dBz_dy_hi;
    ...
    for ( std::size_t y = 1; y < Ny_; ++y ) {
        for ( std::size_t x = 1; x < Nx_; ++x ) {
            for ( std::size_t d = 0; d < t; ++d ) {
                ...
                psi_Exz_[pi_lo] = b_Ez[d] * psi_Exz_[pi_lo] + c_Ez[d] * dBx_dz_lo;
                psi_Eyz_[pi_lo] = b_Ez[d] * psi_Eyz_[pi_lo] + c_Ez[d] * dBy_dz_lo;
                ...
                psi_Exz_[pi_hi] = b_Ez[t - 1 - d] * psi_Exz_[pi_hi] + c_Ez[t - 1 - d] * dBx_dz_hi;
                psi_Eyz_[pi_hi] = b_Ez[t - 1 - d] * psi_Eyz_[pi_hi] + c_Ez[t - 1 - d] * dBy_dz_hi;
